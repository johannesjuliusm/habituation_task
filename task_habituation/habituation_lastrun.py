...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='/Users/jjm/sci/memobaby/task_habituation/habituation_lastrun.py',
...
def run(expInfo, thisExp, win, inputs, globalClock=None, thisSession=None):
    ...
    moving_dots = visual.DotStim(
        ...
        depth=0.0)
    boing_sound = sound.Sound('/Users/jjm/sci/memobaby/task_habituation/stimuli/sounds/boing.mp3', secs=-1, stereo=True, hamming=True,
        name='boing_sound')
    boing_sound.setVolume(1.0)
    key_resp_attention = keyboard.Keyboard()
    ...
    new_stim_rabbit = visual.ImageStim(
        ...
        flipHoriz=False, flipVert=False,
        texRes=128.0, interpolate=True, depth=0.0)
    key_resp_new_stim = keyboard.Keyboard()
    ...
    thisExp.addData('start_screen.started', globalClock.getTime())
    task_title.reset()
    ...
    thisExp.addData('start_screen.stopped', globalClock.getTime())
    # check responses
    ...
    for thisTrial in trials:
        ...
        print(f"Looking time in trial {i}: {looktimes[i]} s")
        
        # calculate baseline looking time from first three trials
        if i == 2:
            baseline_looktime = (looktimes[0] + looktimes[1] + looktimes[2]) / 2
            print(f"Baseline looking time is: {baseline_looktime} s")
        # compare recent looking times to baseline and determine if experiment is finished
        if i >= 5:
            sum_last_three_looktimes = (looktimes[i-2] + looktimes[i-1] + looktimes[i])
            print(f"Cumulative looking time is: {sum_last_three_looktimes} s")
            if sum_last_three_looktimes < baseline_looktime:
        ...
        continueRoutine = True
        # update component parameters for each repeat
        thisExp.addData('attention_grabber.started', globalClock.getTime())
        ...
        for thisComponent in attention_grabberComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        thisExp.addData('attention_grabber.stopped', globalClock.getTime())
    ...
    continueRoutine = True
    # update component parameters for each repeat
    thisExp.addData('new_stim.started', globalClock.getTime())
    ...
    for thisComponent in new_stimComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    thisExp.addData('new_stim.stopped', globalClock.getTime())
    # check responses
